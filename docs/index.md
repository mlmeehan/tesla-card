...
render-polish for the composed Scene + its embedded vehicle cell. It bundles to a single ES module
(`dist/tesla-card.js`) consumed by Home Assistant and distributed via HACS, organized as a
...
_Documentation generated by the BMAD Method `document-project` workflow. Regenerated by a 2026-06-24
**exhaustive, heavy-parallelized** scan (8 concurrent per-slice readers) to reflect **Epics 9–11** and the
`fix/epic9-editor-review-hardening` branch. This pass reconciled stale test counts (was "1,562 / 23 e2e")
to the current **65 files / ~1,578 unit + 24 specs / ~293 e2e**, the energy-key count (21 → **22**), and
corrected the Scene-aware-editor description (there is **no in-editor preview**; the `_isMyHome` seam only
switches which template renders). The "all six `flow/` files frozen" framing is retired in favour of
**AR-6** (only `flow/balance.ts` is the frozen authority; `flow/` is now 8 non-test modules)._
