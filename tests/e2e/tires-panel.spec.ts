// Tires panel — RUNTIME E2E for Story 5.8 (FR-19). The unit gate
// (src/components/panel-tires.test.ts) proves the TS/jsdom logic — the computed
...
// browser, when a user opens the Tires tab:
//   AC1 — four corners each render their live pressure + native unit (bar, 1-dp)
//         under the American "Tire pressure" title; the generic silhouette paints;
...
//
// The Tires panel is NOT the default open panel (charging is), so every test opens
// it first. Sibling panels.spec.ts proves the tab strip + navigation generically;
// this file is the 5.8-specific tires surface. Fixture values (awake corpus):
...
// ── AC1 — four corners render pressure + unit under the American title ──────────
test.describe('AC1 — corners render live pressure + bar unit, American title', () => {
  test('the Tires tab opens and renders four corner cards', async ({ demo }) => {
    await demo.open({ scenario: 'awake' });
    await demo.openPanel('Tires');
    await expect(demo.tiresPanel).toBeVisible();
    await expect(demo.tireCorners).toHaveCount(4);
  });

  test('each corner shows its pressure with the native bar unit (1-dp)', async ({ demo }) => {
    await demo.open({ scenario: 'awake' });
    await demo.openPanel('Tires');
    await expect(demo.tireCorner('fl')).toContainText('2.9');
    await expect(demo.tireCorner('fl')).toContainText('bar');
    await expect(demo.tireCorner('fr')).toContainText('2.6');
  });

  test('the "Tire pressure" title and the generic car silhouette render', async ({ demo }) => {
    await demo.open({ scenario: 'awake' });
    await demo.openPanel('Tires');
    await expect(demo.tiresPanel.getByText('Tire pressure')).toBeVisible();
    await expect(demo.tiresPanel.locator('.car svg')).toBeVisible();
  });
});
...
test.describe('AC2 — TPMS warning surfaces as a text+icon chip, not colour alone', () => {
  test('the TPMS-flagged corner (FR) warns; the other three stay calm', async ({ demo }) => {
    ...
    await expect(demo.tireCorner('fr')).toHaveClass(/warn/);
    await expect(demo.tireWarnChips).toHaveCount(1);
    for (const pos of ['fl', 'rl', 'rr'] as const) {
      await expect(demo.tireCorner(pos)).not.toHaveClass(/warn/);
    }
  });
  ...
  test('the low signal is a "Low" chip (icon + real text), never colour-only', async ({ demo }) => {
    await demo.open({ scenario: 'awake' });
    await demo.openPanel('Tires');
    const chip = demo.tireCorner('fr').locator('.c-warn');
  ...
  test('the head summary reflects the warning honestly ("Check pressure")', async ({ demo }) => {
    await demo.open({ scenario: 'awake' });
    await demo.openPanel('Tires');
    await expect(demo.tiresSummary).toContainText('Check pressure');
  });
});
...
test.describe('AC3 — foreign install renders the panel calmly, no crash', () => {
  test('an unresolved install shows em-dash placeholders, never a false reading', async ({
    ...
    await demo.openPanel('Tires');
    await expect(demo.tiresPanel).toBeVisible();
    await expect(demo.tireCorners).toHaveCount(4); // the four placeholders still paint
    await expect(demo.tireCorner('fl')).toContainText('—'); // NaN-safe neutral placeholder
    ...
    await expect(demo.tireWarnChips).toHaveCount(0);
    await expect(demo.tiresSummary).toContainText('No data');
